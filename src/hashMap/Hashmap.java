...
public class Hashmap<K extends Comparable<K>, V extends Comparable<V>> implements Map<K, V> {
    ...
    int size; // переделать на приватный
    ...
    @Override
    public void put(K k, V v) { // сделала, но ради интереса хочется еще проверить как работает с колизией (с одинаковым хэшом, но разными ключами
        size++; // не всегда
        keys.add(k);
        values.add(v);
    ...
    public void remove(K value) throws NoSuchElementException { // работает с коллизией, проверила
        ...
        if (table[index] == null) { // если в таком индексе пусто, то не существует нода с таким ключом
            throw new NoSuchElementException("такого ключа не существует, подмойся");
    ...
    private void passList(Node<K, V> curNode, K key, Node<K, V>[] nextContainingNode) {
        if (curNode.next == null) {
            throw new NullPointerException("такого ключа не существует, подмойся");
